...
# publication dates
dates <- str_remove_all(publications_txt, "\\n") %>% str_extract_all("<PubDate>.{1,}?</PubDate>")
dates <- dates[[1]]

dates <- dates %>% str_squish() %>% str_remove_all("</?(Year|Month|Day|PubDate)>") %>% str_trim(side = "both")


# abstract texts
abstracts <- str_remove_all(publications_txt, "\\n") %>% str_extract_all("<Abstract>.{1,}?</Abstract>")
abstracts <- abstracts[[1]]

abstracts <- str_remove_all(abstracts, "</?.{1,}?>")
...
```{r create data frame, eval=FALSE}
df <- data.frame(
  PubMed_Id = ids,
  title = titles,
  journal = journals,
